...
The filament is pulled through.The filamanet is pinched between a large 100mm
wheel and a 22x7x8mm bearing. It uses an AS5600 magnetic encoder to read
position.
...
#define CSTR_BUFFER_LEN                                                        \
  32 // don't make this too large. The display need 1024bytes of ram!
...
void do_switches(void) {
  ...
  swmode[1] = swmode[0];
  swmode[0] = (digitalRead(SW1_RESET_PIN) == LOW) &&
              (digitalRead(SW2_UNITS_PIN) == LOW);
...
void doEncoder() {
  ...
  if (angle_delta >= REVOLUTION - WRAP_ZONE ||
      angle_delta <= -REVOLUTION + WRAP_ZONE) {
...
void dfunc(void) { Serial.print("."); }

void updateDisplayNormal(void) {
  display.clearDisplay();
  // draw frame
  display.drawLine(0, 0, 0, display.height() - 1, SSD1306_WHITE);
  display.drawLine(0, display.height() - 1, display.width() - 1,
                   display.height() - 1, SSD1306_WHITE);
  display.drawLine(display.width() - 1, display.height() - 1,
                   display.width() - 1, 0, SSD1306_WHITE);
